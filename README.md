...
```bash
localitas-core app-store add --name homebase --compose ./docker-compose.yml --port 9221
localitas-core app-store start homebase
```

Or open the App Store UI (package icon, top-right nav) and paste the `docker-compose.yml`.

The image is published to `ghcr.io/localitas/localitas-app-homebase:latest`. To publish a new version:
